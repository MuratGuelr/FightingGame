...
class Player {
  ...
  strike(player, enemy, attackDmg) {
    ...
    let damageAmount = Number(Math.ceil(Math.random() * Number(attackDmg)))
    ...
    return console.log(`${player.name} attacks ${enemy.name} for ${damageAmount} damage`)
  }
  // ** Heal the player for random number from  1 to 5 **
  heal(player) {

    // Get random number between 1 - 5 and store that in hpAmount
    let hpAmount = Number(Math.ceil(Math.random() * 5))
    // Add hpAmount to players health
    player.health += hpAmount
    //  Update the game and DOM with updateGame()
    updateGame(p1, p2, game.isOver)
    //  Return a message of 'player name heals for hpAmount HP'
    return console.log(`${player.name} heal himself ${hpAmount} HP!`)
  }
}
...
class Game {
  ...
  let message = ""
    ...
    if (isOver == true && p1.health < p2.health) {
      document.getElementById('victoryPlayer2').play()
    ...
    // Else if isOver is true AND p2 health is <= 0 then update message variable  to 'p2 WINS!'
      ...
      document.getElementById('victoryPlayer1').play()
...
let player1 = new Player("Terry Bogard", 100, 10)
let player2 = new Player("Kyo Kusanagi", 100, 10)
